...
def job(begin,initial_ve,initial_lo,lag_time):
    ...
    array_colname = array_result.columns
    ...
    for i in col_name:
        if i in array_colname:
            continue
        else:
            print('缺少{}列'.format(i))
            ve_initial = initial_ve
            lo_initial = initial_lo
            return ve_initial,lo_initial,begin
    ...
    if analy_data.empty:
        print('时间段{}-{}运行数据为空'.format(shift_begin,shift_end))
        ve_initial = initial_ve
        lo_initial = initial_lo
        return ve_initial,lo_initial,end
    else:
        analy_data_index = analy_data.index
        # analy_data_index2 = analy_data_index.to_series(index=None,name=None)
        # print(analy_data_index2.values)
        ...
        import_data = location_data.set_index(analy_data_index)
        print(type(import_data))
        # import_data = pd.concat([analy_data_index2,location_data],axis = 0)
        import_data.to_csv('D:/analysis/data/import_data.csv', encoding='utf_8_sig', index=True, sep=',')
        # data = df.loc[3:6]
        ve_initial = velocity_data.iloc[-1, :]
        lo_initial = location_data.iloc[-1, :]
        conn = get_Pymysql()
        for idx, row in import_data.iterrows():
            print('导入日期：',idx)
            cursor = conn.cursor()
            cursor.executemany(
                "INSERT INTO dbo.data_2 VALUES (%s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
                [(idx, row['振动风选_1#振动X'], row['振动风选_1#振动Y'], row['振动风选_1#振动Z'],
                  row['振动风选_2#振动X'], row['振动风选_2#振动Y'], row['振动风选_2#振动Z'],
                  row['振动风选_3#振动X'], row['振动风选_3#振动Y'], row['振动风选_3#振动Z'],
                  row['振动风选_4#振动X'], row['振动风选_4#振动Y'], row['振动风选_4#振动Z'],
                  row['振动风选_5#振动X'], row['振动风选_5#振动Y'], row['振动风选_5#振动Z'])])
            conn.commit()
        conn.close()
        return ve_initial, lo_initial, end
...
def main():
    col = 15
    lag_time = 3
    beijing_time = datetime.datetime(2022,11,10,0,0,0)
